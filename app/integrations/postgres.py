...
class PostgresExecutionRepository:
    ...
    async def save(self, execution: Execution) -> Execution:
        async with self.sessions() as session:
            row = ExecutionRow(
                id=execution.id, workflow=execution.workflow, status=execution.status.value,
                input=execution.input, output=execution.output, error=execution.error,
                attempts=execution.attempts, created_at=execution.created_at, updated_at=execution.updated_at,
            )
    ...
    async def get(self, execution_id: str) -> Execution | None:
        async with self.sessions() as session:
            ...
            if row is None:
                return None
            return Execution(
                id=row.id, workflow=row.workflow, status=ExecutionStatus(row.status), input=row.input,
                output=row.output, error=row.error, attempts=row.attempts,
