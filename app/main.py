...
async def demo_run(request: Request) -> str:
    ...
    if result.status.value == "succeeded":
        return demo_page(task, json.dumps(result.output or {}, ensure_ascii=False, indent=2), meta=f"Execution ID: {result.id} · Status: {result.status.value} · Attempts: {result.attempts}")
    details = {"status": result.status.value, "error": result.error, "output": result.output, "attempts": result.attempts}
    return demo_page(task, json.dumps(details, ensure_ascii=False, indent=2), meta=f"Execution ID: {result.id} · Status: {result.status.value} · Attempts: {result.attempts}", error=True)
