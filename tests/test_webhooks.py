...
@pytest.mark.asyncio
async def test_webhook_matches_rule_and_enqueues_execution(monkeypatch: pytest.MonkeyPatch) -> None:
    rule = AutomationRule(
        id=uuid4(),
        name="welcome-new-customer",
        trigger={"type": "customer.created"},
        conditions=[RuleCondition(field="customer.name", operator="exists")],
        actions=[RuleAction(type="prepare_message", parameters={"message": "Welcome {{customer.name}}"})],
    )
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        response = await client.post(
            ...
            headers={"X-Webhook-Secret": "secret", "X-Webhook-Id": "evt-123"},
...
async def test_webhook_rejects_invalid_secret(monkeypatch: pytest.MonkeyPatch) -> None:
    rule = AutomationRule(
        id=uuid4(),
        name="protected-rule",
        actions=[RuleAction(type="prepare_message", parameters={"message": "ok"})],
    )
    await rule_store.save(rule)
    monkeypatch.setattr("app.core.config.settings.inbound_webhook_secret", "secret")
    ...
    transport = httpx.ASGITransport(app=app)
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        response = await client.post(f"/api/webhooks/{rule.id}", json={"type": "test"})
